...
- (void)viewDidLoad {
    ...
    //是否有描述信息
    if (self.displayDescribe) {
        describe = [UILabel new];
        describe.backgroundColor= [UIColor colorWithRed:0.1 green:0.1 blue:0.1 alpha:0.7];
...
- (void)layoutVisiblePages {
    ...
    //描述
    if (self.displayDescribe && describe) {
        int height =44;
        height = self.navigationController.navigationBar.frame.size.height;
        if (height==0) {
            height = 44;
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            if(self.indexFormat){
                self.title = [NSString stringWithFormat:self.indexFormat, (unsigned long)(_currentPageIndex+1), (unsigned long)numberOfPhotos];
            }else{
    ...
    if (describe&&numberOfPhotos>1) {
        if ([_delegate respondsToSelector:@selector(photoBrowser:describeForPhotoAtIndex:)]) {
            describe.text = [_delegate photoBrowser:self describeForPhotoAtIndex:_currentPageIndex];
        }
    }
